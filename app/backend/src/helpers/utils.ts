...
export const leaderboard = `SELECT
name AS name,
SUM(totalPoints) AS totalPoints,
SUM(totalGames) AS totalGames, SUM(totalVictories) AS totalVictories,
SUM(totalDraws) AS totalDraws, SUM(totalLosses) AS totalLosses,
SUM(goalsFavor) AS goalsFavor, SUM(goalsOwn) AS goalsOwn, SUM(goalsBalance) AS goalsBalance,
...
WHERE m.in_progress = 0
GROUP BY name
ORDER BY name, totalPoints DESC, totalVictories DESC,
goalsBalance DESC, goalsFavor DESC, goalsOwn DESC) AS classification
GROUP BY name
ORDER BY totalPoints DESC, totalVictories DESC, goalsBalance DESC, goalsFavor DESC, goalsOwn DESC;`;
